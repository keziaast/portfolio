// src/components/Contact.js

import { contactInfo } from '../lib/data'

// Ikon berwarna per platform — sesuai referensi website Sufyan
const contactIcons = {
  whatsapp: {
    bg: 'bg-green-500',
    hover: 'group-hover:bg-green-600',
    svg: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
      </svg>
    ),
  },
  email: {
    bg: 'bg-white border border-gray-200 dark:border-white/10',
    hover: 'group-hover:border-[#0BB8D4]',
    // Gmail colored M logo
    svg: (
      <svg viewBox="0 0 24 24" className="w-7 h-7">
        <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.910 1.528-1.145C21.69 2.28 24 3.434 24 5.457z" fill="#EA4335"/>
        <path d="M0 5.457v13.909c0 .904.732 1.636 1.636 1.636h3.819V11.73L0 5.457z" fill="#34A853"/>
        <path d="M18.545 11.73v9.273h3.819A1.636 1.636 0 0 0 24 19.366V5.457L18.545 11.73z" fill="#4285F4"/>
        <path d="M12 9.548L5.455 4.64 0 5.457l12 9.091 12-9.091-5.455-.817L12 9.548z" fill="#FBBC05"/>
      </svg>
    ),
  },
  telegram: {
    bg: 'bg-[#229ED9]',
    hover: 'group-hover:bg-[#1a8bc2]',
    svg: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
        <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
      </svg>
    ),
  },
  linkedin: {
    bg: 'bg-[#0A66C2]',
    hover: 'group-hover:bg-[#084fa0]',
    svg: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
      </svg>
    ),
  },
  instagram: {
    bg: 'bg-gradient-to-br from-[#f9ce34] via-[#ee2a7b] to-[#6228d7]',
    hover: 'group-hover:opacity-90',
    svg: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>
      </svg>
    ),
  },
  github: {
    bg: 'bg-[#24292e] dark:bg-[#333]',
    hover: 'group-hover:bg-[#1a1e22]',
    svg: (
      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
        <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/>
      </svg>
    ),
  },
  spotify: {
  bg: 'bg-[#1DB954]',
  hover: 'group-hover:bg-[#1aa34a]',
  svg: (
    <svg viewBox="0 0 24 24" className="w-7 h-7" fill="white">
      <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/>
    </svg>
  ),
},
}

export default function Contact() {
  return (
    <section id="contact" className="py-24 pb-32 px-[6%] max-w-[1180px] mx-auto">
      <div className="reveal">
        <span className="sec-tag">Hubungi Saya</span>
        <h2 className="text-3xl md:text-4xl font-bold mt-2 mb-1">
          Feel Free To <span className="text-[#0BB8D4]">Contact Me</span> 📨
        </h2>
        <div className="sec-line" />
        <p className="text-sm text-gray-500 dark:text-[#7878A0] mb-10 max-w-lg">
          Saya selalu terbuka untuk berdiskusi, berkolaborasi, atau sekadar menyapa.
          Hubungi saya melalui platform berikut.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 reveal">
        {contactInfo.map(contact => {
          const ic = contactIcons[contact.icon] || contactIcons.github

          return (
            <a key={contact.label}
               href={contact.url}
               target="_blank"
               rel="noopener noreferrer"
               className="group card p-5 flex items-center gap-4
                          hover:-translate-y-1 hover:shadow-lg
                          hover:border-[#0BB8D4] transition-all duration-200">

              {/* Ikon berwarna */}
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center
                               flex-shrink-0 transition-all duration-200
                               ${ic.bg} ${ic.hover}`}>
                {ic.svg}
              </div>

              {/* Info */}
              <div className="min-w-0">
                <div className="text-xs font-bold text-[#1A1A2E] dark:text-[#E8E8F5] mb-0.5">
                  {contact.label}
                </div>
                <div className="text-xs text-gray-500 dark:text-[#7878A0] truncate mb-1.5">
                  {contact.value}
                </div>
                <span className="text-xs font-semibold text-[#0BB8D4]
                                 group-hover:underline transition-all">
                  Send Me A Message →
                </span>
              </div>
            </a>
          )
        })}
      </div>

      {/* Peta Kampus */}
<div className="mt-12 reveal">
  <p className="text-xs font-semibold text-gray-500 dark:text-[#7878A0] mb-3 uppercase tracking-wider">
    📍 Lokasi Kampus
  </p>
  <div className="card overflow-hidden">
    <div className="bg-gray-50 dark:bg-[#13131E] px-5 py-3 border-b border-gray-200
                    dark:border-white/[0.07] flex items-center gap-2">
      <span className="text-[#0BB8D4] text-sm font-semibold">
        Universitas Sains dan Teknologi Indonesia  {/* ← ganti nama kampus */}
      </span>
    </div>
    <iframe 
    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3989.6962550445382!2d101.3983311!3d0.44818600000000003!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x31d5a91147214257%3A0x38bf6247b6b4b787!2sUSTI%20Pekanbaru!5e0!3m2!1sid!2sid!4v1776157614317!5m2!1sid!2sid"  
      width="100%"
      height="380"
      style={{ border: 0 }}
      allowFullScreen=""
      loading="lazy"
      referrerPolicy="no-referrer-when-downgrade"
      title="Lokasi Kampus"
    ></iframe>
  </div>
</div>
    </section>
  )
}
